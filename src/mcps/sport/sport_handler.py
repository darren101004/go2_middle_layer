    ...
    def handle(self, request: SportRequest) -> Response:
        ...
        try:
            if request.option == SportOption.DAMP:
                ...
                return Response(success=False, message=f"Sport command {request.option} not found", data=None, code=404)
            return Response(success=True, message=f"Sport command {request.option} executed successfully", data=None, code=200)
        except Exception as e:
            return Response(success=False, message=str(e), data=None, code=500)
